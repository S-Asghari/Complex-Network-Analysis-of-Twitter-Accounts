...
def removing_outliers_0(l, num_of_removals):

    middle = (np.percentile(l, 25) + np.median(l) + np.percentile(l, 75)) / 3
    n = len(l)
    d = [abs(l[i] - middle) for i in range(n)]
...
def degree_distribution(G):
    ...
    l1_fake = [in_degrees.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l1_genu = [in_degrees.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    l1_genu = removing_outliers_0(l1_genu, 2)

    axis[0].hist([l1_fake, l1_genu], bins=32, color=['tomato', 'limegreen'], label=['fake', 'genuine'])
    axis[0].legend(loc='upper right')
    axis[0].set_xlabel('in-degree')
    axis[0].set_ylabel('node count')
    axis[0].set_title('in-degree distribution')

    l2_fake = [out_degrees.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l2_genu = [out_degrees.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    l2_genu = removing_outliers_0(l2_genu, 2)

    axis[1].hist([l2_fake, l2_genu], bins=32, color=['tomato', 'limegreen'], label=['fake', 'genuine'])
...
def betweenness_centrality(G):
    ...
    l_fake = [betweenness_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [betweenness_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    l_genu = removing_outliers_0(l_genu, 12)

    plt.hist([l_fake, l_genu], bins=32, color=['tomato', 'limegreen'], label=['fake', 'genuine'])
...
def pagerank(G):
    ...
    l_fake = [pagerank.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [pagerank.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    l_genu = removing_outliers_0(l_genu, 12)
...
def closeness_centrality(G):
    ...
    l_fake = [closeness_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [closeness_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    # l_genu = removing_outliers_0(l_genu, 1)
...
def eigenvector_centrality(G):
    ...
    l_fake = [eigenvector_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [eigenvector_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    l_genu = removing_outliers_0(l_genu, 11)
...
def katz_centrality(G):
    ...
    l_fake = [katz_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [katz_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    l_genu = removing_outliers_0(l_genu, 12)
...
def clustering_coefficient(G):
    ...
    l_fake = [clustering_coefficient.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [clustering_coefficient.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
...
def average_path_length(G):
    ...
    l_fake = [avg_path_len.get(k) for k in G.nodes if nx.get_node_attributes(G, 'node_label')[k] == 0]
    l_genu = [avg_path_len.get(k) for k in G.nodes if nx.get_node_attributes(G, 'node_label')[k] == 1]
    l_fake = removing_outliers_0(l_fake, 1)
    l_genu = removing_outliers_0(l_genu, 1)
...
def harmonic_centrality(G):
    ...
    l_fake = [harmonic_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [harmonic_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
    # l_genu = removing_outliers_0(l_genu, 1)
...
def local_reaching_centrality(G):
    ...
    l_fake = [l_reaching_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G, 'node_label')[k] == 0]
    l_genu = [l_reaching_centrality.get(k) for k in G.nodes if nx.get_node_attributes(G, 'node_label')[k] == 1]
...
def average_neighbor_degree(G):
    ...
    l_fake = [avg_neighbor_degree.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 0]
    l_genu = [avg_neighbor_degree.get(k) for k in G.nodes if nx.get_node_attributes(G,'node_label')[k] == 1]
...
def removing_outliers(x, y, num_of_removals):
    ...
    n = len(x)
    d = [math.sqrt(pow((x[i] - x_median)/x_normalizer, 2) + pow((y[i] - y_median)/y_normalizer, 2)) for i in range(n)]
    for i in range(num_of_removals):
        max_index = d.index(max(d))
        del d[max_index]
        del x[max_index]
